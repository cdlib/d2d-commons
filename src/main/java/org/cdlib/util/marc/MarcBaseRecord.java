...
/**
 * Class representing a MARC record. Functionality includes parsing MARC records from raw input,
 * building a MARC record from individual fields, and providing access to the fields that comprise
 * the MARC record.
 *
...
public class MarcBaseRecord implements MarcConstants {
   ...
   */
  private static Logger log = LoggerFactory.getLogger(MarcBaseRecord.class);
  ...
  /**
   * Private indicator used to assure CVS information is logged only once.
   */
   ...
   *
   * This Vector should be replaced with a linked list of the Fields, MarcFieldLists, but that is a
   * significant rewrite, and so must wait until somebody with the right combination of bugdet,
   * vision, and gumption, comes along.
   *
   * Note to future programmers: choose your data structures well; those that follow you will live
   * with the quality of your decisions, for good, or for ill.
   */
  ...
  /**
   * Ths status of this marc record. The valid values are: MARC_REC_NOT_BUILT when the record is
   * incomplete, MARC_REC_CLEAN when the record is complete and clean, and MARC_REC_ALHPA_TAGS when
   * the record contains some non-numeric tags.
   *
  ...
  /**
   * Insert a new field at the end of its nXX group, e.g. a new 650 field would be placed at the end
   * of any exising 6xx fields, before the first 7xx field.
   */
  ...
  /**
   * Instantiate a new <code>MarcBaseRecord</code> from the supplied <code>MarcBaseRecord</code>.
   */
  ...
  /**
   * Instatiate a new <code>MarcBaseRecord</code> using the fields in the supplied
   * <code>MarcFieldList</code> using the insertion order specified.
   *
  ...
  /**
   * Create a new <code>MarcBaseRecord</code> from a string of raw marc data, inserting fields in the
   * specified order.
   *
  ...
  /**
   * Create a new <code>MarcBaseRecord</code> from a string of raw marc data. Insertion order is
   * defaulted to END_LIST.
   *
   * @param rawMarc string of marc data
   *
   * @exception MarcFormatException when the the raw marc data cannot be parsed
  ...
  /**
   * Reset the fields of this object from those in the supplied <code>MarcFieldList</code> using the
   * insertion order specified.
   *
   * @param list <code>MarcFieldList</code> to use
   * @param order field insertion order to use
   *
  ...
  /**
   * Populate the fields for this marc record object by parsing the supplied <code>String</code>.
   * Fields are insterted in the order in which they appear in the <code>String</code>.
   *
   * @param rawMarc string of marc data
   *
  ...
  /**
   * Parse the raw marc data in the supplied <code>String</code> to create the fields for this marc
   * objects. If this object already contains fields, those fields are cleared. This method checks for
   * and end of record character in the last byte of the data buffer as indicated by the leader record
   * length (the first five bytes of the marc data), and for the end of field character at the end of
   * each field as defined by the directory offset and length.
   *
   * @param rawMarc string of marc data
   * @param order field insertion order to use
   *
  ...
  /**
   * Get the status code for this record object. This should not be confused with marc status in the
   * leader record. The object status is used to indicate whether the record has been built, and if it
   * includes non-numeric field tags.
   *
  ...
  /**
   * Return the media code and description stings for this record using the record type and biblevel
   * values to form a lookup key for the validMedia HashMap.
   *
  ...
  /**
   * Creates an xml encoded string version of this marc record object. This feature has never been
   * fleshed out and should be considered experimental.
   *
  ...
  /**
   * Delete each field given in the supplied <code>MarcFieldList</code> from this marc record. The
   * fields are matched using the <code>Field</code> attribute <code>id</code>.
   *
   ...
   *
   * @return MarcFieldList of fields whose tags are in the specified range of startTag through endTag,
   *         returns an empty list if there are no matching fields.
   */
  ...
  /**
   * Find all instances of a field with a specified range of tags. Tags are specifed with their
   * numeric values to reduce needless <code>String</code> to <code>int</code> conversions.
   *
   * @param startTag the beginning tag in the range
   * @param endTag the ending tag in the range
   *
   * @return MarcFieldList of fields whose tags are in the specified range of startTag through endTag,
   *         returns an empty list if there are no matching fields. Only field with numeric tags are
   *         returned.
   */
  ...
  /**
   * Find first instance of a field with the specified tag. Tag is specifed with its numeric value to
   * reduce needless <code>String</code> to <code>int</code> conversions.
   *
   * @param tag the tag to select
   *
   * @return The first Field with the specified tag, or null if there is none.
  ...
  /**
   * Return the first instance of the specified subfield in the first instance of the specified field
   * found in the record. Most useful when this is known in advance to be unique.
   *
   * @param tag the tag to select
   ...
   *
   * @throws IllegalArgumentException if the tag is less than "010", and is therefore not a variable
   *         length field, or if the subfield code is either null or not of length one.
   *
   *
  ...
  /**
   * Find all instances of a field whose tag and data match the supplied regular expressions.
   *
   * @param tag a regular expression to match the tag, e.g. "3\d5" finds 305, 315; null or "" matches
   *        all tags
   *
   * @param data a regular expression used searched within field content; null or "" matches all
   *        content
   *
   * @return MarcFieldList of fields with mathcing tag and data elements or null if there are no
   *         matching fields.
   *
   ...
   *        content
   *
   * @param matchFlags flags used to modify the regular expression matching, e.g.
   *        RE.MATCH_CASEINDEPENDENT will make the match case independent
   *
   * @return MarcFieldList of fields with mathcing tag and data elements or null if there are no
   *         matching fields.
   *
  ...
  /**
   * Check the bibliographic level in the leader to see if this is a monograph record. Note: this test
   * is not adequate for all sources.
   *
  ...
  /**
   * Check the bibliographic level in the leader to see if this is a serial record. Note: this test is
   * not adequate for all sources.
   *
  ...
  /**
   * Replace fields based on field id in list. If the field is not found then add the field using the
  ...
  public void replaceFields(MarcFieldList list, int order) {
    ...
    for (int i = 0; i < max; i++) {
      ...
      } else { // field not found so add field using order
  ...
  /**
   * Set the bibliographic codes in the leader. The supplied <code>String</code> should be 1 to 5
   * characters in length. The supplied <code>String</code> is used to overlay the leader value
   * beginning at offset 5, for the lenght of the <code>String</code>.
   *
  ...
  /**
   * Add a copy of the supplied field to this marc record using the given insertion order.
   *
  ...
  /**
   * Add the supplied field to this marc record using the given insertion order.
   *
   * @param f the field to add
   * @param order the insertion order to use
   *
  ...
  /**
   * Add the Fields in the supplied <code>MarcFieldList</code> to this marc record using the supplied
   * insertion order. The Fields are copied prior to being added, so changes to the original set of
   * fields will not affect the fields in this marc record object.
   *
   * @param list <code>MarcFieldList</code> to use
   * @param order field insertion order to use
   *
  ...
  /**
   * Format the contents of this Marc record into a human readable form and return the whole thing in
   * a String.
   *
   * @return string containing formatted output
   */
  ...
  /**
   * Apply character translations to this records variable length field's subfield's values using the
  ...
  /**
   * Build a map keyed by field tag with the number of occurrences of that tag within this record and
   * return that map. Because the map is <code>TreeMap</code> the entires may be retrieved in tag
   * order.
   *
  ...
  /**
   * Build a map of subfields and their counts for a particular field within this record.
   *
   * @return the map of subfield occurrence counts
   */
  ...
  /**
   * Build a map of subfields and their counts for a set of fields within this record. The result is a
   * map of maps keyed by field tag.
   *
   * @return the map of subfield occurrence counts maps The data type structure is String x Map(String
   *         x Integer)
   */
  ...
  /**
   * Build a map of subfields and their counts for all of the fields within this record. The result is
   * a map of maps keyed by field tag.
   *
   * @return the map of subfield occurrence counts maps The data type structure is String x Map(String
   *         x Integer)
   */
  ...
  /**
   * Accumulate the values of a one map into another map. Both maps must be keyed by the same type and
   * have values of type Integer.
   *
  ...
  /**
   * A private method used during debugging to dump the contents of a Map object to the log.
   *
   ...
   *
   * Why this method is in this class, and not a MarcFieldList constructor is a real mystery.
   *
  ...
  /**
   * Return nominal record length, i.e., length as recorded in leader
   */
  ...
  /**
   * Return actual record length, i.e., sum of lengths of leader, directory, and fields
   */
  ...
  /**
   * Find the field with the matching field id and return its index into the fields
   * <code>Vector</code>. If no match is found, return -1.
   *
